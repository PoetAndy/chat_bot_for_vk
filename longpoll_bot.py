...
from text_generator import GenMessage

class LongPollBot(Bot, GenMessage):
    """
    ...
    def run_long_poll(self):
        ...
        for event in self.long_poll.listen():
            ...
            if event.type == VkEventType.MESSAGE_NEW and event.to_me and event.text:
                ...
                self.send_message(receiver_user_id=event.user_id, message_text=self.generate_message(promt=event.text))

                # self.send_message(receiver_user_id=event.user_id, message_text=self.generate_message(promt=event.text)) # Отправка на генерацию ответа по API
